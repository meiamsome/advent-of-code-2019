...
fn get_shortest_path<'a>(ancestors: HashMap<&'a str, Vec<&'a str>>, from: &str, to: &str) -> usize {
    ...
    from_list.symmetric_difference(&to_list).collect::<Vec<_>>().len()
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{:?}", get_shortest_path(
        get_planet_ancestors(&contents.trim()),
        "YOU",
        "SAN"
    ));
    Ok(())
...
mod test {
    ...
    fn test_get_shortest_path() {
        ...
        assert_eq!(
            get_shortest_path(ancestors, "YOU", "SAN"),
            4
        );
    }
