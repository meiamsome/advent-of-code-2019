...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write!(screen, "{}{}{}", termion::cursor::Goto(1, 1), termion::clear::CurrentLine, termion::cursor::Hide)?;
    loop {
        if let Some(x) = vm.next() {
            if let Some(y) = vm.next() {
                if let Some(tile_id_or_score) = vm.next() {
                    if x == -1 {
                        write!(screen, "{}Score: {}{}", termion::cursor::Goto(1, 1), tile_id_or_score, termion::cursor::Hide)?;
                    } else {
